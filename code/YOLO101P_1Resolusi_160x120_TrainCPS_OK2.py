...
def load_videos(k_val, Rx=12, Ry=20, RzFaktor=8):
...
#
    ...
    return (train_set_R1, Y_train), (test_set_R1, Y_test)
...
with open(filenya, 'w') as out_file:
    ...
    for k_val in range(1,2):
        ...
        (x_train, y_train), (x_test, y_test) = load_videos(k_val, Rx, Ry, RzFaktor)

        scenario_name = str(Rx) + '_x_' + str(Ry) + '_x_' + str(RzFaktor)

        input_shape = (13, 120, 160, 3)
        ...
        conv1tMP = layers.MaxPooling3D(pool_size=(1, 2, 4), strides=None, padding='valid', data_format=None)(conv1t)
        ...
        conv1MP = layers.MaxPooling3D(pool_size=(1, 4, 4), strides=None, padding='valid', data_format=None)(conv1)
        ...
        decoder.add(layers.Dense(np.prod(input_shape)/768, activation='relu'))
        decoder.add(layers.Reshape(target_shape=(52/(RzFaktor),Rx/32,Ry/24,3), name='out_recon_unupsized1'))
        ...
        decoder.add(layers.Conv3D(filters=16, kernel_size=(3,5,5), strides=1, padding='same', activation='relu', name='conv3Dout0'))
        decoder.add(layers.BatchNormalization(epsilon=0.001, axis=-1, momentum=0.99, weights=None, beta_init='zero', gamma_init='one', gamma_regularizer=None, beta_regularizer=None))
        decoder.add(layers.UpSampling3D(size=(1, 3, 4)))
        decoder.add(layers.Conv3D(filters=8, kernel_size=(3,5,5), strides=1, padding='same', activation='relu', name='conv3Dout1'))
        decoder.add(layers.BatchNormalization(epsilon=0.001, axis=-1, momentum=0.99, weights=None, beta_init='zero', gamma_init='one', gamma_regularizer=None, beta_regularizer=None))
        ...
        model.fit(x = [x_train, y_train], y = [y_train, x_train],
        ...
        Y_pred = model.predict([x_test, y_test], batch_size = 8)
