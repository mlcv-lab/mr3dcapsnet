...
vartuning = 'C3D_1Resolusi_112x112'
import csv
...
with open(filenya, 'w') as out_file:
    ...
    for k_val in range(8,9):
        ...
        gc.collect()
        (x_train, x_train2, y_train), (x_test, x_test2, y_test) = load_videos(k_val, Rx1, Ry1, Rx2, Ry2, RzFaktor)

        scenario_name = "C3D_" + str(Rx1) + 'x' + str(Ry1) + '_' + str(RzFaktor)
        ...
        print "sebelum popping layers 3cd ================"
        model_c3d.summary()
        ...
        model_c3d.pop()
        model_c3d.pop()
        print "setelah popping layers 3cd ==============="
        model_c3d.summary()        
        ...
        primarycaps = PrimaryCap(func_c3d, dim_capsule=4, n_channels=8, kernel_size=(1,3,3), strides=2, padding='valid')
        ...
        decoder.add(layers.Dense(np.prod(input_shape)/256, activation='relu', input_dim=4*n_class))
        decoder.add(layers.Reshape(target_shape=(32/(RzFaktor*4),112/8,Rx1/8,3), name='out_recon_unupsized1'))
        decoder.add(layers.UpSampling3D(size=(2, 2, 2)))
        decoder.add(layers.Conv3D(filters=32, kernel_size=(3,3,3), strides=1, padding='same', activation='sigmoid', name='conv3Dout0'))
        decoder.add(layers.UpSampling3D(size=(1, 2, 2)))
        decoder.add(layers.Conv3D(filters=16, kernel_size=(3,3,3), strides=1, padding='same', activation='sigmoid', name='conv3Dout1'))
        decoder.add(layers.UpSampling3D(size=(2, 2, 2)))
        ...
        tb = callbacks.TensorBoard(log_dir='./sken4a', batch_size=2, histogram_freq=0, write_graph=True,
                                   write_images=True)
        log = callbacks.CSVLogger('./result' + '/log' + '_' + scenario_name + '_validasiK_' + str(k_val) + '_' + '.csv')
