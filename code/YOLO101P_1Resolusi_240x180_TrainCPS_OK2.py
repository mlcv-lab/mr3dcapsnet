...
def PrimaryCap(inputs, dim_capsule, n_channels, kernel_size, strides, padding):
    ...
    conv1Caps = layers.MaxPooling3D(pool_size=(2, 3, 4), strides=None, padding='valid', data_format=None)(outputC)
...
with open(filenya, 'w') as out_file:
    ...
    for k_val in range(1,2):
        ...
        conv1tMP = layers.MaxPooling3D(pool_size=(1, 3, 4), strides=None, padding='valid', data_format=None)(conv1t)
        ...
        conv1MP = layers.MaxPooling3D(pool_size=(1, 4, 3), strides=None, padding='valid', data_format=None)(conv1)
        ...
        primarycaps = PrimaryCap(conv1DO, dim_capsule=8, n_channels=16, kernel_size=(3,5,5), strides=1, padding='valid')
        ...
        decoder.add(layers.Dense(np.prod(input_shape)/1728, activation='relu', input_dim=4*n_class))
        decoder.add(layers.Reshape(target_shape=(52/(RzFaktor),Ry/36,Rx/48,3), name='out_recon_unupsized1'))
        ...
        decoder.add(layers.BatchNormalization(epsilon=0.001, axis=-1, momentum=0.99, weights=None, beta_init='zero', gamma_init='one', gamma_regularizer=None, beta_regularizer=None))
        decoder.add(layers.UpSampling3D(size=(1, 4, 3)))
