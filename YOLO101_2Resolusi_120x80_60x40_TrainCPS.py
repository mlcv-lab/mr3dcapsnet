...
def PrimaryCap(inputs, dim_capsule, n_channels, kernel_size, strides, padding, nameK):
    ...
    conv1Caps = layers.MaxPooling3D(pool_size=(2, 2, 2), strides=None, padding='valid', data_format=None)(outputC)
...
# --------------------------------------------- input datanya mnist
import csv
vartuning = '2Resolusi_120x80_60x40'
...
with open(filenya, 'w') as out_file:
    ...
    for k_val in range(1,2):
        ...
        conv1t_1 = layers.Conv3D(filters=32, kernel_size=(3,5,5), strides=1, padding='valid', activation='relu', name='conv1t_1')(x1)
        conv1tMP_1 = layers.MaxPooling3D(pool_size=(1, 2, 3), strides=None, padding='valid', data_format=None)(conv1t_1)
        ...
        conv1MP_1 = layers.MaxPooling3D(pool_size=(1, 2, 2), strides=None, padding='valid', data_format=None)(conv1_1)
        ...
        primarycaps_1 = PrimaryCap(conv1DO_1, dim_capsule=8, n_channels=16, kernel_size=(3,3,3), strides=1, padding='valid', nameK = "1")

        x2 = layers.Input(shape=input_shape2)

        # Layer 1: Just a conventional Conv2D layer
        conv1t_2 = layers.Conv3D(filters=32, kernel_size=(3,5,5), strides=1, padding='valid', activation='relu', name='conv1t_2')(x2)
        conv1tMP_2 = layers.MaxPooling3D(pool_size=(1, 2, 3), strides=None, padding='valid', data_format=None)(conv1t_2)
        ...
        conv1_2 = layers.Conv3D(filters=32, kernel_size=(3,5,5), strides=1, padding='valid', activation='relu', name='conv1_2')(conv1tDO_2)
        conv1MP_2 = layers.MaxPooling3D(pool_size=(1, 4, 4), strides=None, padding='valid', data_format=None)(conv1_2)
        ...
        # Layer 2: Conv2D layer with `squash` activation, then reshape to [None, num_capsule, dim_capsule]
        primarycaps_2 = PrimaryCap(conv1DO_2, dim_capsule=8, n_channels=16, kernel_size=(3,3,3), strides=1, padding='valid', nameK = "2")
        ...
        decoder.add(layers.Dense(np.prod(input_shape2)/384, activation='relu'))
        decoder.add(layers.Reshape(target_shape=(52/(RzFaktor),Rx2/24,Ry2/16,3), name='out_recon_unupsized1'))
        ...
        decoder.add(layers.UpSampling3D(size=(1, 4, 4)))        
